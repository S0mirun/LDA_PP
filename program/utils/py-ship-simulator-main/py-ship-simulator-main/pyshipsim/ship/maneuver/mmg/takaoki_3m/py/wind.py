...
def irregularWind(wind_velo_ave, physical_time):
    ...
    for i in range(div_omega):
        ...
        spectrum_wind[i] = 4.0 * k_wind * np.average(
            wind_velo_ave)**2/omega_wind[i]+xd_wind**2 / (1.0+xd_wind**2)**(4.0/3.0)
    ...
    for j in range(np.size(physical_time)):
        for i in range(div_omega):
            wind_velo_instant[j] += amp_wind[i] * \
                np.sin(omega_wind[i] * physical_time[j] +
...
def windForce(UA, AOA, AT, AL, Lz, rhoA, lpp, XX0, XX1, XX3, XX5, YY1, YY3, YY5, NN1, NN2, NN3, KK1, KK2, KK3, KK5):
    ...
    CXwind = XX0 + XX1 * np.cos(AOA) + XX3 * \
        np.cos(3.0 * AOA) + XX5 * np.cos(5.0 * AOA)
    CYwind = YY1 * np.sin(AOA) + YY3 * np.sin(3.0 * AOA) + \
        YY5 * np.sin(5.0 * AOA)
    CNwind = NN1 * np.sin(AOA) + NN2 * np.sin(2.0 * AOA) + \
        NN3 * np.sin(3.0 * AOA)
    CKwind = KK1 * np.sin(AOA) + KK2 * np.sin(2.0 * AOA) + \
        KK3 * np.sin(3.0 * AOA) + KK5 * np.sin(5.0 * AOA)
...
def windCoeff(lpp, AT, AL, AOD, LCW, LCBR, HBR, HC, SBW, LZ):
    ...
    XX0 = X00 + X01 * (SBW * HBR / AT) + X02 * \
        (LCW / HC) + X03 * (AOD / lpp / lpp)
    ...
    YY1 = Y10 + Y11 * (LCBR / lpp) + Y12 * (LCW / lpp) + Y13 * \
        (AL / AOD) + Y14 * (LCW / HC) + Y15 * (AT / (SBW * HBR))
    YY3 = Y30 + Y31 * (AL / (lpp * SBW)) + Y32 * (lpp * HC / AL) + Y33 * (
        LCBR / lpp) + Y34 * (SBW / HBR) + Y35 * (AOD / AL) + Y36 * (AT / (SBW * HBR))
    YY5 = Y50 + Y51 * (AL / (lpp * SBW)) + Y52 * (lpp / HBR) + Y53 * (LCBR / lpp) + \
        Y54 * (SBW ** 2 / AT) + Y55 * (LCW / lpp) + Y56 * (LCW * HC / AL)

    NN1 = N10 + N11 * (LCW / lpp) + N12 * (lpp * HC / AL) + N13 * (AT / AL) + N14 * (LCW / HC) + \
        N15 * (AL / (lpp * SBW)) + N16 * (AT / lpp ** 2) + \
        N17 * (SBW ** 2 / AT) + N18 * (LCBR / lpp)
    NN2 = N20 + N21 * (LCBR / lpp) + N22 * (LCW / lpp) + N23 * (AL / AOD) + N24 * (AT / SBW ** 2) + \
        N25 * (lpp / HBR) + N26 * (AT / (SBW * HBR)) + N27 * \
        (AL / (lpp * SBW)) + N28 * (AL / lpp ** 2)
    NN3 = N30 + N31 * (LCBR / lpp) + N32 * (AT / (SBW * HBR)) + N33 * (AL / AT)

    KK1 = K10 + K11 * (HBR / lpp) + K12 * (AT / (lpp * SBW)) + K13 * (lpp * HC / AL) + K14 * (
        LCW / lpp) + K15 * (LCBR / lpp) + K16 * (SBW / HBR) + K17 * (SBW ** 2 / AT) + K18 * (lpp / SBW)
    KK2 = K20 + K21 * (SBW / HBR) + K22 * (AT / SBW ** 2) + K23 * (AL / (lpp * HC)) + K24 * \
        (LCBR / lpp) + K25 * (HBR * LCW / AL) + \
        K26 * (lpp / SBW) + K27 * (lpp ** 2 / AL)
    KK3 = K30 + K31 * (SBW ** 2 / AT) + K32 * (LCBR / lpp) + K33 * (HC / lpp) + \
        K34 * (AT / (lpp * SBW)) + K35 * \
        (lpp * SBW / AL) + K36 * (AOD / lpp ** 2)
    KK5 = K50 + K51 * (AL / (lpp * HC)) + K52 * (AL / AOD) + K53 * \
        (AT / AL) + K54 * (lpp / SBW) + K55 * (AL / (lpp * SBW))
