...
from . import wind
from . import hullforce
from . import propellerforce
from . import rudderforce
...
class MMGModel(object):
    def __init__(self):
        init_const = pd.read_csv(
            f"{__dir__}/input_csv/principal_particulars.csv",
            header=0,
            index_col=0,
        )
        init_parameter = pd.read_csv(
            f"{__dir__}/input_csv/MMG_params.csv", header=0, index_col=0
        )
        model_switch = pd.read_csv(
            f"{__dir__}/input_csv/model_switch.csv", header=0, index_col=0
        )
        #
        self.t = 0.0
    ...
    def ode_rhs(self, x, u, w):
        ...
        delta_rudder = np.array([delta_rudder])
    ...
    def hydroForceLs(
        ...
        u_nd = np.where(
            np.abs(U_ship) < 1.0e-5, 1.0e-7 * np.ones(u_velo.shape), u_velo / U_ship
        )
        v_nd = np.where(
        ...
        U_ship = np.where(
            np.abs(U_ship) < 1.0e-5, 1.0e-7 * np.ones(U_ship.shape), U_ship
        )

        ##########################
        ### Force of Hull ###
        ##########################
        hull_force = hullforce.hullForceYoshimuraSimpson(
            U_ship,
            u_velo,
            vm_velo,
            r_angvelo,
        ...
        ### Force of Propeller ###
        ##########################
        prop_force = propellerforce.get_propellerforce(
            u_velo,
            vm_velo,
            r_angvelo,
            n_prop,
        ...
        ### Force of Ruder ###
        ##########################
        rudder_force = rudderforce.get_rudderforce(
            u_velo,
            vm_velo,
            r_angvelo,
            delta_rudder,
            ...
            self.gammaP,
            self.gammaN,
            self.kx_rudder_reverse,
            self.cpr_rudder,
            self.coeff_urudder_zero,
            XP,
            self.switch_rudder,
            self.switch_fn_rudder,
        ...
        ### compute instant relative(apparent) wind dirction and speed
            ...
            if self.switch_windtype == 1:  # uniform
                ...
                wind_relative = relative.true2Apparent(
                ...
                wind_velo_true_instant = wind.irregularWind(
                    wind_velo_true, physical_time
                )
                wind_dir_true_instant = np.copy(wind_dir_true)
                ### compute relative wind on midship
                wind_relative = relative.true2Apparent(
