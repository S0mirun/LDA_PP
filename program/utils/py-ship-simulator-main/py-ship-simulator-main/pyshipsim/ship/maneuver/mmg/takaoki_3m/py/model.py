...
from .propellerforce import get_propellerforce_kang
from .rudderforce_vectwin import get_rudderforceVecTwin
...
class MMGModel(object):
    def __init__(self):
        ...
        # AT
        self.area_projected_trans = init_const.at["area_projected_trans", "value"]
        # AL
        self.area_projected_lateral = init_const.at["area_projected_lateral", "value"]
        # AOD
        self.area_projected_lateral_superstructure = init_const.at["AOD", "value"]
        # C or LCW, midship to center of AL
        self.lcw = init_const.at["LCW", "value"]
        # CBR, midship to center of AOD(superstructure or bridge)
        self.lcbr = init_const.at["LCBR", "value"]
        # Height from free surface to top of the superstructure (bridge) (m)
        self.hbr = init_const.at["HBR", "value"]
        # Hc, hight of center of lateral projected area
        self.hc_AL = init_const.at["HC", "value"]
        # ship breadth fro wind force computation
        self.breadth_wind = init_const.at["SBW", "value"]
        # Lz --- Acting position of sway force from center of gravity (it is necessary to calculate roll moment)
        self.swayforce_to_cg = init_const.at["Lz", "value"]
        ...
        self.kt_coeff = np.array(
            [
                init_parameter.at["kt_coeff0", "params_init"],
                init_parameter.at["kt_coeff1", "params_init"],
        ...
        # 1.5  coeff. from exp
        self.alpha_p = init_parameter.at["alpha_prop", "params_init"]
        ...
        self.switch_prop_x = model_switch.at["switch_prop_x", "value"]
        # 0: YP=0 at n>0, 1: use exp.polynomial for 2nd q
        self.switch_prop_y = model_switch.at["switch_prop_y", "value"]
        # 0: NP=0 at n>0, 1: use exp.polynomial for 2nd q
        self.switch_prop_n = model_switch.at["switch_prop_n", "value"]
        # 0: YR & NR are 0 at n<0, 1: compute force at n<0
        self.switch_rudder = model_switch.at["switch_rudder", "value"]
        # model for rudder normal force, 1: Fujii, 2: lindenburg
        self.switch_fn_rudder = model_switch.at["switch_fn", "value"]
        # model for rudder normal force, 1: CFD, 2: Kang
        self.switch_cn_rudder = model_switch.at["switch_cn", "value"]
        # model for effective inflow angle of rudder, 1: standard MMG by Yoshimura, 2: reversed model by Yasukawa on u<0
        self.switch_ur_rudder = model_switch.at["switch_ur", "value"]
        # 0: off, 1: on
        self.switch_wind = model_switch.at["switch_wind", "value"]
        # 1: uniform wind, 2: irregular wind, 3: instantaneous wind measured by exp (converted to true), 4: use exp relavite wind
        self.switch_windtype = model_switch.at["switch_windtype", "value"]
    ...
    def hydroForceLs(
        ...
        ### Force of Propeller ###
        ##########################
        prop_force = get_propellerforce_kang(
            u_velo,
            vm_velo,
            r_angvelo,
            n_prop,
        ...
        one_minus_t_prop = prop_force[:, 5:6]
