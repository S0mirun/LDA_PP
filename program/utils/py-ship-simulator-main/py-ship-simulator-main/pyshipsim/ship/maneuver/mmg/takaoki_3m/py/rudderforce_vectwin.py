...
def get_rudderforceVecTwin(
    ...
    xh_rudder_nd = np.where(
        ...
        -0.159,
        np.where(ah_rudder_cal < -0.4344, -0.4344, ah_rudder_cal),
    )
    ...
    VecTwin_Cns = pd.read_csv(
        f"{__dir__}/input_csv/VecTwin_Cns.csv", header=0, index_col=0
    )
    VecTwin_Cnp = pd.read_csv(
        f"{__dir__}/input_csv/VecTwin_Cnp.csv", header=0, index_col=0
    )
