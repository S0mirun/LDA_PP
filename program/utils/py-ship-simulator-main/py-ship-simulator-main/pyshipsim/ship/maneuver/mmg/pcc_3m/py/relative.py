...
def true2Apparent(UT, U_ship, zeta_wind, beta_hat, psi):
    ...
    chi_beta = np.where(
        chi_beta < 0,
        chi_beta + 2.0 * np.pi,
        np.where(chi_beta > 2.0 * np.pi, chi_beta - 2.0 * np.pi, chi_beta),
    )

    # compute apparent wind speed
    UA = np.sqrt(UT**2 + U_ship**2 - 2.0 * UT * U_ship * np.cos(chi_beta))
    U_ship = np.where(np.abs(U_ship) < 1e-8, 1e-8, U_ship)
    # use intermidiate value rv To avoid the value larger than 1.0 due to numerical error on acos.
    _rv = np.where(UA == 0.0, 0.0, (U_ship**2 + UA**2 - UT**2) / (2.0 * U_ship * UA))
    ...
    gamma_beta = np.where(chi_beta <= np.pi, np.arccos(rv), 2.0 * np.pi - np.arccos(rv))
    gamma_a = gamma_beta + beta_hat
    gamma_a = np.where(
        np.abs(U_ship) < 1.0e-5, (3 * np.pi - _chi) % (2 * np.pi), gamma_a
    )
    gamma_a = np.where(np.abs(UT) < 1.0e-5, beta_hat, gamma_a)
    gamma_a = np.where(gamma_a > 2.0 * np.pi, gamma_a - 2.0 * np.pi, gamma_a)
    gamma_a = np.where(gamma_a < 0.0, gamma_a + 2.0 * np.pi, gamma_a)
...
def apparent2True(UA, U_ship, gamma_a, beta_hat, psi):
    ...
    gamma_beta = np.where(
        ...
        np.where(gamma_beta > 2.0 * np.pi, gamma_beta - 2.0 * np.pi, gamma_beta),
    )
    ...
    chi_beta = np.where(gamma_beta <= np.pi, np.arccos(rv), 2.0 * np.pi - np.arccos(rv))

    zeta_wind = psi - chi_beta + beta_hat + np.pi
    zeta_wind = np.where(np.abs(UT) < 1.0e-5, 0.0, zeta_wind)
    zeta_wind = np.where(zeta_wind > 2.0 * np.pi, zeta_wind - 2.0 * np.pi, zeta_wind)
    zeta_wind = np.where(zeta_wind < 0.0, zeta_wind + 2.0 * np.pi, zeta_wind)
