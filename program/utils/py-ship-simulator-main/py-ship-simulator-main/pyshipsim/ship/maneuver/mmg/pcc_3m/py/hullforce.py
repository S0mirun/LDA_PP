...
def hullForceYoshimura(
    ...
    for j in range(vm_velo.size):
        for i in range(i_max - 1):
            ...
            Y_ad[j, 0] = (
                Y_ad[j, 0]
                + 0.5 * (np.abs(comp0) * comp0 + np.abs(comp1) * comp1) / i_max
            )
            N_ad[j, 0] = (
                N_ad[j, 0]
                + 0.5
                * (np.abs(comp2) * comp2 * x0 + np.abs(comp3) * comp3 * x1)
                / i_max
            )
        YHN[j, 0] = -CD * Y_ad[j, 0]
        NHN[j, 0] = -CD * N_ad[j, 0]
    ...
    XH = (
        ...
        * draft
        * (
            (X_0F_nd + (X_0A_nd - X_0F_nd) * (np.abs(beta) / np.pi)) * u_velo * U_ship
            + Xvr_nd * vm_velo * r_angvelo * lpp
        )
    )
    YH = (
        0.5
        ...
        * lpp
        * draft
        * (Yv_nd * vm_velo * np.abs(u_velo) + Yr_nd * r_angvelo * lpp * u_velo + YHN)
    )
    NH = (
        0.5
        * rho_fresh
        * lpp**2
        * draft
        * (Nv_nd * vm_velo * u_velo + Nr_nd * r_angvelo * lpp * np.abs(u_velo) + NHN)
    )
    hullforce = np.concatenate((XH, YH, NH), axis=1)
    return hullforce


def hullForceYoshimuraSimpson(
    U_ship,
    u_velo,
    vm_velo,
    r_angvelo,
    ...
    YH = (
        ...
        * draft
        * (
            Yv_nd * vm_velo * np.abs(u_velo)
            + Yr_nd * r_angvelo * lpp * u_velo
            - y_clossflow
        )
    )
    NH = (
        ...
        * draft
        * (
            Nv_nd * vm_velo * u_velo
            + Nr_nd * r_angvelo * lpp * np.abs(u_velo)
            - n_clossflow
        )
    )
    hullforce = np.concatenate([XH, YH, NH], axis=1)
