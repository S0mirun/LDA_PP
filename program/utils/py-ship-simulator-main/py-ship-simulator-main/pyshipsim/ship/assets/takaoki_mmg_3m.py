...
from ..maneuver.mmg import takaoki_3m
...
class Takaoki3mMMG(SurfaceShip):

    def __init__(self, solve_method: str = "euler", f2py: bool = True):
        super().__init__(solve_method)
        if f2py and hasattr(takaoki_3m, "f2pyMMGModel"):
            self.maneuvering_model = takaoki_3m.f2pyMMGModel()
        elif f2py and not hasattr(takaoki_3m, "f2pyMMGModel"):
            msg = "You can't use takaoki_3m's f2py mpdule. Please compile fortran code."
            warnings.warn(msg)
            self.maneuvering_model = takaoki_3m.pyMMGModel()
        else:
            self.maneuvering_model = takaoki_3m.pyMMGModel()
        #
        self.reg_principal_particular(L=3.0, B=0.4843)
    ...
    def ode_rhs(self, state: Variables, action: Variables) -> Variables:
        x, u, w = state[0:6], action[0:4], action[4:6]
...
class Takaoki3mMMGWithActuaorResponse(Takaoki3mMMG):
    def __init__(self, solve_method: str = "euler", f2py: bool = True):
