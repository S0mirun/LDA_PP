...
from ..maneuver.mmg import esso_osaka_3m
...
class EssoOsaka3mMMG(SurfaceShip):
    def __init__(self, solve_method: str = "euler", f2py: bool = True):
        super().__init__(solve_method)
        if f2py and hasattr(esso_osaka_3m, "f2pyMMGModel"):
            self.maneuvering_model = esso_osaka_3m.f2pyMMGModel()
        elif f2py and not hasattr(esso_osaka_3m, "f2pyMMGModel"):
            warnings.warn(
                "You can't use Esso_osaka_3m's f2py mpdule. Please compile fortran code."
            )
            self.maneuvering_model = esso_osaka_3m.pyMMGModel()
        else:
            self.maneuvering_model = esso_osaka_3m.pyMMGModel()
        #
...
class EssoOsaka3mMMGWithActuaorResponse(EssoOsaka3mMMG):
    def __init__(self, solve_method: str = "euler", f2py: bool = True):
